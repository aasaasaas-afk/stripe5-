...
def process_payment(card_input):
    ...
    if "failureType" in apx1:
        ...
        return f"{ccn}|{mm}|{yy}|{cvc} --> {apl}"
    else:
        return f"{ccn}|{mm}|{yy}|{cvc} --> PAYMENT_SUCCESS"

@app.route('/', methods=['GET'])
def gateway():
    if request.args.get('gateway') == 'stripe5$' and request.args.get('key') == 'rocky' and 'cc' in request.args:
...
if __name__ == "__main__":
    #modify if u can bsdk (by: xcracker911)
    print("Stripe Charge 1$")
    app.run(debug=True)
